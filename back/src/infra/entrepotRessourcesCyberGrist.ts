  ...
  implements EntrepotRessourcesCyber
{
  ...
  async tous(): Promise<RessourceCyber[]> {
    ...
    return reponse.records.map((record) => ({
      ...
      thematiques: this.aseptiseListe(record.fields.Thematiques),
      publicsCible: this.aseptiseListe(record.fields.Cible),
      niveaux: this.aseptiseListe(record.fields.Cycle_si_eleves),
      types: this.aseptiseListe(record.fields.Type),
      besoins: this.aseptiseListe(record.fields.Besoins),
